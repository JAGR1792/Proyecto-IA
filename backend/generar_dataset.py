...
Uso:
    python generar_dataset.py --real      # Descarga GTFS real (requiere internet)
...
def main():
    ...
    try:
        if args.real:
            print(">>> GENERANDO DATASET TAXIS REAL (GTFS OFICIAL) <<<")
            print("Esto descargará ~50-100MB y puede tardar varios minutos...")
            grafo = repo.generar_dataset_inicial_taxis(usar_gtfs_real=True, descargar_geo=True)
        elif args.osm:
            ...
            grafo = repo.generar_dataset_inicial_taxis(usar_gtfs_real=False)
